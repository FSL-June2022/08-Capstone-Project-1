...
router.post("/add-meeting", verifyToken, async (req, res) => {
    try{
        let meeting = new datastore(req.body);
        let result = await meeting.save();
        res.status(200).json(result);
    } catch (error) {
        res.status(500).json(error.message);
    }
...
router.get("/meetings", verifyToken, async (req, res) => {
    try {
        let meetings = await datastore.find();
        if(meetings.length > 0){
            res.status(200).json(meetings);
        }else{
            res.send({result : "NO Meetings Found "})
        }
    } catch (error) {
        res.status(500).json(error.message);
    }
